    ...
    if args.cats is not None:
        if ":" in args.cats:
            for s in args.cats.split(";"):
                if ":" in s:
                    parts = s.split(":", 1)
                    merged_cats.append(parts[0])
                    cat_map[parts[0]] = parts[1]
                cats_parsed.append(s)
        else:
        ...
        # Parse --cats, either mapping or list
            ...
            if ":" in args.cats:
                ...
                for cat in cats_parsed:
                    ...
                    mcat = cat.split(":", 1)[0]
                    cats = sum(
                        [fnmatch.filter(available_channels, _cat) for _cat in cat_map[mcat].split(",")],
                        [],
                    )
        ...
        for fittype, channel, blind, blind_range, sname, label in zip(
            ...
            if label is None:
                if len(channel) < 6:
                    ...
                    label = cat_map.get(sname, 1) if args.cats and ":" in args.cats else 1
