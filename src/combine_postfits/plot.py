...
def plot(fitDiag_uproot, 
         ...
         project_signal: list[float | int] | None = None,  # Plot projected total signal on x-axis, scale by `project_signal`
         remove_tiny: bool | float | int | str = False,  # Remove enteries with total yield below treshold eg. {False | 10 | '5%'}
         # Style opts
    ...
    if len(bkgs) == 0:
        bkgs = default_bkgs
    if len(sigs) > 2:
        raise ValueErrors("Are you insane? More than 2 signal? Write your own plotter lol.")
    elif len(sigs) == 0:
        sigs = default_signal
    ...
    unused = [key for key in hist_keys if key not in sigs+bkgs+project+[onto] and 'total' not in key]
    if len(unused) > 0:
    ...
    if onto is None:
        ...
        hep.histplot(hist_dict_fcn(onto), ax=ax,
                     ...
                     histtype='step', color=style['qcd']['color'],# hatch=style['qcd']['hatch'],
                     lw=2,
                    )
    ...
    if project_signal is not None:
        ...
        if fitDiag_root is None and "prefit" not in fit_type:
            ...
            if fitDiag_root is not None and rmap is None:
                raise ValueError("Cannot infer sample:poi mapping for more than 1 signal. Pass e.g. " 
                                 "rmap={'hbb': 'r', 'hcc':'r2'}"
                    )
            else:
                _rs = {sig: get_fit_val(fitDiag_root, rmap[sig], fittype=fit_type, substitute=1.) for sig in rmap}
        ...
        for sig in sigs:
            ...
            hep.histplot(_scaled_sig, ax=ax, 
...
#                 )
    ...
    if np.sum(~good_yerr_mask) > 1:
        logging.warning(f"  Bkg. Unc. in {np.sum(~good_yerr_mask)} bins is too large ( > err_th) and will be set to 0. Full uncertainty is: {yerr_nom}")
    ...
    # Ratio
    rax.legend(loc='upper right', ncol=3,
...
#     rax.legend(reversed(handles), reversed(labels), loc='upper right', ncol=2)
    ...
    if fit_type == 'prefit':
        ...
        if fitDiag_root is None:
            ...
            mu_strs = []
            for sig in sigs:   
                _r = get_fit_val(fitDiag_root, rmap[sig], fittype=fit_type, substitute=1.)
                _r = fr"{_r:.2f}"
                _d, _u = get_fit_unc(fitDiag_root, rmap[sig], fittype=fit_type, substitute=1.)
                _d, _u = fr"{_d:.2f}", f"{_u:.2f}"
                mu_strs.append(r"$\mu_{%s}$ = " % style[sig]['label'].replace("$", "")
                               + r"${%s}_{%s}^{%s}$" % (_r, _d, _u))
    ...
    if chi2:
        chi2 = np.sum(np.nan_to_num(abs(data.values() - tot.values())/data.variances(), posinf=0, neginf=0))
        # Should be just a bit higher than 'saturated'
        at = AnchoredText(fr"$\chi^2$ = {chi2:.2f}",
                          loc='upper left', #pad=0.8,
                          prop=dict(size='x-small', ha='center'), 
                          frameon=False,
                          )
