...
def make_style_dict_yaml(fitDiag, cmap="tab10", sort=True, sort_peaky=False):
    ...
    def linearity(h):
        ...
        # ⚡ Bolt: Replace np.polyfit with manual vectorized calculation (approx 3x faster)
        # Avoids generalized SVD routines from numpy when we just need 1D linear regression
        sum_x = np.sum(x)
        sum_y = np.sum(_h)
        sum_xx = np.sum(x * x)
        sum_xy = np.sum(x * _h)
