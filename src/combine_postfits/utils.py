...
def make_style_dict_yaml(fitDiag, cmap="tab10", sort=True, sort_peaky=False):
    ...
    temp_linearity_lists = {k: [] for k in sample_keys}
        ...
        for ch in avail_channels:
            ...
            for k in ch_dir.keys(cycle=False):
                ...
                if hasattr(obj, "to_hist"):
                    ...
                    yield_dict[k] += np.sum(vals)
                    temp_linearity_lists[k].append(linearity(hist_obj))

    linearity_dict = {k: np.mean(lst + [0]) for k, lst in temp_linearity_lists.items()}
    sort_score_dicts = {}
