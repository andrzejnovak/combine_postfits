...
def make_style_dict_yaml(fitDiag, cmap="tab10", sort=True, sort_peaky=False):
    ...
    def get_samples_fitDiag(fitDiag):
        ...
        for fit in avail_fit_types:
            try:
                ...
                for ch in [ch[:-2] for ch in fit_dir if ch.count("/") == 0]:
                    if ch in fit_dir:
                        snames.update(k[:-2] for k in fit_dir[ch].keys())
            except KeyError:
    ...
    def linearity(h):
        ...
        if denominator == 0:
            return 0
        m = (n * np.sum(x * _h) - sum_x * sum_y) / denominator
        c = (sum_y - m * sum_x) / n
        fy = m * x + c
    ...
    for fit in avail_fit_types:
        ...
        for ch in avail_channels:
            ...
            for key in ch_dir.keys(cycle=False):
                if "total" in key or key not in sample_keys:
                    continue
                obj = ch_dir[key]
