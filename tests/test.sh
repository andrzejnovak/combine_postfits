...
if [ "$1" = "C" ] || [ "$1" = "*" ]; then
    combine_postfits --dpi $DPI -i fitDiags/fit_diag_C.root -o outs/plots_C_all --toys    -p $P
    combine_postfits --dpi $DPI -i fitDiags/fit_diag_C.root -o outs/plots_C --toys  --style styles/style_C.yml --xlabel 'Jet $m_{reg}$'  
fi
